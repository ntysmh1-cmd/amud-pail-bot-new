...
bot.on('callback_query', async (query) => {
  ...
  } else if (query.data === 'preview_page') {
    try {
      const expireDate = Math.floor(Date.now() / 1000) + 90;
      const link = await bot.createChatInviteLink(CHANNELS.previewId, {
        expire_date: expireDate,
        member_limit: 1,
        creates_join_request: false,
        name: 'p-' + userId + '-' + Date.now()
      });

      await bot.sendMessage(chatId, `👀 קישור הצצה אישי (תוקף 90 שניות, לשימוש חד פעמי):\n\n${link.invite_link}\n\n⚠️ אל תשתף. הקישור יימחק אוטומטית.`);

      // Kick the user after 70 seconds if they joined
      setTimeout(async () => {
        try {
          await bot.banChatMember(CHANNELS.previewId, userId);
          await bot.unbanChatMember(CHANNELS.previewId, userId); // so they can re-join later with new link
          console.log('Kicked preview user', userId);
        } catch (err) {
      ...
      bot.sendMessage(chatId, `❌ לא הצלחתי ליצור קישור זמני.\nשגיאה: ${e.message}`);
    }
  }
});
