...
bot.on('callback_query', async (query) => {
  ...
  if (query.data === 'buy_75') {
    ...
    try {
      const expireDate = Math.floor(Date.now() / 1000) + 70; // 70 seconds
      const link = await bot.createChatInviteLink(CHANNELS.previewId, {
        expire_date: expireDate,
        member_limit: 1,
        name: 'preview-' + Date.now()
      });
      bot.sendMessage(chatId, `👀 קישור הצצה זמני (תוקף כדקה אחת):\n${link.invite_link}\n\n⚠️ הקישור יפוג אוטומטית. אל תשתף אותו.`);
    } catch (e) {
      console.log('Preview error:', e.message);
      bot.sendMessage(chatId, `👀 קישור הצצה:\n${CHANNELS.preview}\n\n(שגיאה ביצירת קישור זמני: ${e.message})`);
    }
  }
});
